...
The most common specialty is Surgery, followed by Consult - History and Phy., Cardiovascular / Pulmonary, Orthopedic, and Radiology. In general, the distribution is imbalanced, i.e., a few specialties have a lot of samples, and the rest have very small counts.
...
It is very obvious that punctuations and stopwords are the most frequent words in the data. Without removing them, we cannot find out medical themes.
...
When we remove stopwords, we can spot some medical themes, such as "patient", "medical", "clinical", etc. In general, it gives us a better idea of what the text is about because the documents right now is more medical-related/meaningful.
...
There are 806 clinical stopwords. I removed "left", "right", "normal", and "noted" in addtion. When using clinical stopwords, we can spot more medical themes then the scenario using general stopwords. The updated top stop words are more towards the medical domain.
...
The biagrams make sense. They are mostly common clinical phrases, such as "operation room", "blood pressure" etc. Therefore, the biagrams capture meaningful medical themes then single words.
...
For the words before "blood", they are mostly measurement terms, such as "estimated", "signs".
For the words after "blood", they are mostly standard medical terms such as "pressure", "loss", etc, and some lab terms such as "cell(s)", "sugar(s)", etc.
...
(ggplot(mt_samples[mt_samples['avg_sentence_length'] > 0], aes(x='reorder(medical_specialty, avg_sentence_length)', y='avg_sentence_length'))  # Hint: aggregated by specialty
...
Average sentence length is similar across most medical specialties. Most medians are around 15-25 words. A few medical specialties show more variabilitiy with some extremely long sentences as outliers, likely reflecting complex medical narratives or detailed clinical notes instead of simple clinical notes.
...
I think it is clearer when there are 5 topics. The medical themes are separated into more clinical areas and made the topic look more distinct, whereas 3 topics is more general and less distinct since it might mix up different clinical areas.
...
The topic model discovers themes that resemble "Cardiovascular / Pulmonary", "Consult - History and Phy.", "Orthopedic", "Surgery", and "Radiology". These topics are mostly related to the medical specialties in the cross-tab. However, personally, I feels Radiology is more mixed, suggesting some shared vocabs across different medical specialties.