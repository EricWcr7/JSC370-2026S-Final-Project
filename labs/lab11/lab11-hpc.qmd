...
**Solutions:** (Refer to the qmd file if unstructured in the html)
...
#SBATCH --partition=jsc370       # use the jsc370 partition
#SBATCH --ntasks=1
...
**Solutions:**
The compute node reports 24 CPU cores in total. It is different from the 1 core you requested because the node physically has 24 cores, but SLURM only gave the job access to 1 of them. SLURM can share a single node among multiple jobs, each getting a slice of the available resources.
...
#SBATCH --partition=jsc370
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=4   # fill in
#SBATCH --mem=4G
#SBATCH --time=00:15:00
#SBATCH --output=flu_4core_%j.out
#SBATCH --error=flu_4core_%j.err

python3 flu_parallel.py 450 4   # fill in
```
...
Job 203392 on squid01 | CPUs allocated: 1
...
Job 203393 on squid01 | CPUs allocated: 4
...
|--------|----------------|----------------|------------------|---------|
| `flu_1core.sh` | 1 | 2.85 | 2.59 | 1.10x |
| `flu_4core.sh` | 4 | 2.73 | 0.74 | 3.70x |

*Is the speedup on the cluster similar to what you observed on your laptop in Lab 10? What factors might cause differences?*

Yes. The 4-core cluster job achieved a speedup of about 3.70x, which is quite similar to the 4-worker result from Lab 10 on my laptop (3.24x for the XGBoost grid search). In both cases, the task was large enough and parallelizable enough for multiprocessing to help a lot, but not perfectly linearly. Differences can come from hardware and environment. The cluster node may have faster or more consistent CPUs, different memory bandwidth, and fewer background processes than a laptop. There is also still overhead from creating worker processes, scheduling tasks, and moving data between processes, so even on the cluster the speedup is not exactly 4x.
...
#SBATCH --partition=jsc370
#SBATCH --array=0-4          # fill in: launch tasks 0 through 4
#SBATCH --ntasks=1
...
As vaccination coverage increases, the mean number of deaths by step 20 decreases. The decline is modest from 0% to 50% vaccination, but then deaths drop much more sharply at higher coverage. The largest drop occurs between 50% and 75% vaccination, where the mean deceased falls from 40.67 to 15.59. By 100% vaccination, deaths are almost eliminated (1.21 on average).
...
![Vaccination sweep: mean deceased at step 20](flu_cluster_results.png)

As vaccination coverage increases, deaths decline gently at first, then drop sharply once coverage gets high enough. The biggest change happens around 75% vaccination, where the outbreak becomes much less severe. At 100% vaccination, deaths are nearly zero. This suggests herd immunity is a threshold effect. Once enough people are vaccinated, transmission is disrupted. So even the overall outbreak dies out, protecting the population as a whole.
...
SLURM manages the cluster by scheduling jobs and assigning resources. We can’t SSH straight into a compute node because those nodes are shared and controlled by SLURM to keep access fair and prevent users from afdecting each other’s jobs.

2. What is a SLURM job array, and when is it more convenient than writing separate batch scripts?

**Solutions:**

It is a way to run multiple similar jobs from one script. It’s more convenient when the jobs use the same code but different inputs or parameter settings.
...
The cluster speedup was about 3.70x, and my laptop speedup in Lab 10 was about 3.24x, so they were pretty similar. The difference could be due to better hardware on the cluster, like faster CPUs, more total cores, and less background activity than on my laptop (since many apps were opened on mine while doing the lab).
...
About 14.25 (2.85, the one core time with 100 simulations, then times 5) seconds serially on one core.  With the job array, it took about 1.38 seconds of wall-clock time (5 levels running at the same time, pick the slowest one as the wall-clock time, according to the previous output).
