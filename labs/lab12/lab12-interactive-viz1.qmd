...
sb_locs_state = sb_locs_state.merge(usa_pop_abbr, left_on="State/Province", right_on="Abbreviation", how="left")

display(sb_locs_state.describe())
```
...
fig = px.scatter(
    ...
    x="population",
    y="n_stores",
    hover_data=["State", "population", "n_stores"],
    text="Abbreviation",
    labels={"population": "Population of a State", "n_stores": "Number of Starbucks"},
    title="Number of Starbucks vs. Population of a State"
)
...
Yes. The number of Starbucks is proportional to the population of a state. States with larger populations tend to have more Starbucks.
...
fig = px.histogram(
    ...
    title="Caloric Distribution of Starbucks Menu Items by Category"
)
fig.show()
```
...
Yes. Food items generally have higher calories and a wider spread, while drinks are concentrated at lower calories.
...
fig = px.bar(
    top20_food.sort_values("count"),
    x="count",
    y="word",
    orientation="h",
    labels={"count": "Count", "word": "Word"},
    title="Top 20 Words in Food Starbucks Menu Items",
    height=600
)
fig.show()
```
...
Here are the top 20 words in Food Starbucks menu items: sandwich, chocolate, egg, cake, cookie, chicken, salad, protein, bagel, box, roasted, breakfast, cheese, turkey, free, croissant, butter, bar, double, pop.
...
fig = px.scatter(
    ...
    color="Category",
    trendline="ols",
    labels={"Carb. (g)": "Carbohydrates (g)", "Calories": "Calories", "Category": "Category"},
    title="Calories vs. Carbohydrates by Category"
)
fig.show()
```
...
Yes. There is a positive relationship between carbs and calories for both categories. Food items tend to have more calories than drinks at similar carbs.
...
fig = px.box(
    ...
    x="top_word",
    y="Calories",
    labels={"top_word": "Top Word", "Calories": "Calories"},
    title="Calorie Content by Top 10 Item Words"
)
fig.show()
```
...
Yes. Food items tend to cluster at higher calories and protein, while drinks cluster at lower proteins. There is also a positive trend between calories and carbs.
...
fig1 = go.Figure(go.Choropleth(
    locations=sb_locs_state["Abbreviation"],
    z=sb_locs_state["n_stores"],
    locationmode="USA-states",
    text=sb_locs_state["hover"],
    hoverinfo="text",
    colorscale="Blues",
    colorbar_title="# Stores",
))
fig1.update_layout(
...
The two maps are similar, with larger population states, such as California, Texas, Florida, and New York, showing high values in both maps. A key difference is that store counts are more concentrated in a few states, while population is more evenly distributed across large states comparing to store counts.
