...
X_train_c, X_test_c, Y_train_c, Y_test_c = train_test_split(
  X_class, Y_class, test_size=0.30, random_state=65
)
...
train_counts = Y_train_c.value_counts().sort_index()
print(train_counts)
print(f"\nProportions:")
train_props = Y_train_c.value_counts(normalize=True).sort_index()
print(train_props)
```
...
log_alphas = np.unique(np.concatenate(([0.0], log_alphas)))
...
(
  ...
  scale_x_log10() +
  labs(
    title='Cost-Complexity Pruning Path (Classification Tree)',
    x='ccp_alpha (log scale)',
    y='Number of leaves'
  ) +
  theme_minimal()
)
...
optimal_ccp_alpha_c = 1.843922e-03
expected_leaves_c = int(
    ccp_df.loc[np.isclose(ccp_df['ccp_alpha'], optimal_ccp_alpha_c), 'n_leaves'].iloc[0]
)
print(f"Chosen ccp_alpha: {optimal_ccp_alpha_c}")
print(f"Expected number of leaves: {expected_leaves_c}")
```
...
According to the confusion matrix, **Private for-profit** is the easiest category to predict by the pruned tree because the model correctly identified `{python} int(cm[class_labels.index("Private for-profit"), class_labels.index("Private for-profit")])` out of `{python} int(cm[class_labels.index("Private for-profit"), :].sum())` workers, which is `{python} f"{float(cm[class_labels.index('Private for-profit'), class_labels.index('Private for-profit')] / cm[class_labels.index('Private for-profit'), :].sum() * 100):.2f}"`%. However, **Unemployed**, **Self-employed**, and **Government** are the hardest categories to predict because the pruned tree cannot correctly identify any of the workers in these categories.
...
# print(Y_train_c.isnull().sum())
# print(X_test_c.isnull().sum())
# print(Y_test_c.isnull().sum())
```
...
rf_oob_acc_c = rf_class.oob_score_

print(f"\nRandom Forest OOB accuracy: {rf_oob_acc_c}")
print(f"Random Forest OOB error: {1 - rf_oob_acc_c}")
...
According to the feature importance plot, the top 3 most important features are `AGE`, `FIRMSIZE`, and `educ_cat`. These features align with the variables that appeared in the pruned classification tree. These features align with the variables that appeared in the pruned classification tree.
...
According to the comparison table, the random forest performs better than the pruned tree on the test set. The random forest has a test accuracy of `{python} round(rf_test_acc_c * 100, 2)`% and a test misclassification error of `{python} round((1 - rf_test_acc_c) * 100, 2)`%, while the pruned tree has a test accuracy of `{python} round(test_acc_c * 100, 2)`% and a test misclassification error of `{python} round(test_error_c * 100, 2)`%. This is expected because Random Forests reduce variance by averaging many decorrelated trees. A single decision tree is prone to high variance and can overfit (or underfit if pruned too heavily), whereas the ensemble approach of Random Forest captures more complex patterns and generalizes better to unseen data.
...
X_train_r, X_test_r, Y_train_r, Y_test_r = train_test_split(
    X_reg, Y_reg, test_size=0.30, random_state=65
)

print(f"Training set size: {X_train_r.shape[0]}")
print(f"Testing set size: {X_test_r.shape[0]}")
print(f"\nINCWAGE in training set:")
print(f"  Mean: ${Y_train_r.mean():.2f}")
print(f"  Std:  ${Y_train_r.std():.2f}")
print(f"  Min:  ${Y_train_r.min():.2f}")
print(f"  Max:  ${Y_train_r.max():.2f}")
```
...
log_alphas_r = np.unique(np.concatenate(([0.0], log_alphas_r)))
...
(
    ...
    scale_x_log10() +
    labs(
        title='Cost-Complexity Pruning Path (Regression Tree)',
        x='ccp_alpha (log scale)',
        y='Number of leaves'
    ) +
    theme_minimal()
...
```{python}
# Choose a ccp_alpha that gives a small, interpretable tree
optimal_ccp_alpha_r = 5.553071e+07
expected_leaves_r = int(
    ccp_df_r.loc[np.isclose(ccp_df_r['ccp_alpha'], optimal_ccp_alpha_r), 'n_leaves'].iloc[0]
)
print(f"Chosen ccp_alpha: {optimal_ccp_alpha_r}")
print(f"Expected number of leaves: {expected_leaves_r}")
```
...
print(f"\nTest RMSE (pruned tree): ${test_rmse_tree:,.2f}")
...
rf_oob_r2 = rf_reg.oob_score_

print(f"\nRandom Forest OOB R-squared: {rf_oob_r2}")
...
According to the feature importance plot, the top 3 most important features are `educ_cat`, `faminc_cat`, and `AGE`. These features make substantive sense because they are all related to the worker's income.
...
I would recommend the random forest because it has a higher test R-squared and lower test RMSE. This means that the random forest is able to capture more variance in the data and is able to generalize better to unseen data.
